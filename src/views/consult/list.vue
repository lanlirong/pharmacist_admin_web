            ...
            >
                ...
                <el-table-column label="操作" width="140" fixed="right">
                    <template slot-scope="{ row }">
                        <router-link :to="`/consult/detail?id=${row.id}&raw=0`" target="_blank">
                            <el-button type="text" size="mini">预览</el-button></router-link
                        >
                        <router-link :to="`/consult/update?id=${row.id}&raw=0`" target="_blank">
...
<script>
import { SELECT_TYPE } from '@/utils/constant/consult';
import { _getList, _deleteOne } from '@/services/api/consult';
export default {
    name: 'consultList',
    data() {
        return {
